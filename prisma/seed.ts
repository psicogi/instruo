...
async function main() {
  ...
  // Cria instrutor
    const instrutor = await prisma.instrutor.create({
        data: {
            ...
            veiculos: {
                create: [
                { tipo: 'CARRO', modelo: 'VW Gol',          categoriaCnh: 'B', valorAula: 120, ativo: true },
                { tipo: 'MOTO',  modelo: 'Honda CG 160 Start', categoriaCnh: 'A', valorAula: 90,  ativo: true },
                ],
            },
        },
    })
  ...
  // Busca veículos
    const veiculos = await prisma.veiculo.findMany({ where: { instrutorId: instrutor.id } })
  ...
  // Cria conta de acesso
    ...
    await prisma.contaInstrutor.create({
        data: {
  ...
  // Cria disponibilidade (Seg a Sáb, 7h às 18h)
